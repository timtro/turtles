...
TEST_CASE(
  ...
  REQUIRE(final.value.x == Approx(initial.x).margin(delta));
  REQUIRE(final.value.y == Approx(initial.y).margin(delta));
  REQUIRE(final.value.th == Approx(initial.th).margin(delta));
  REQUIRE(final.log == std::string{manualLog});
}
